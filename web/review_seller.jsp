...
<script>
   $(document).ready(function(){
       $(".btn.btn-danger").click(function(){
           ...
           var x=$(this).attr("rel");
           var p=("c_"+v+"_"+x).trim();
           $.post("order_cancel.jsp",{id:v,scode:x},function(data){
               $("#"+p).fadeOut(10);
           });   
       });
   });   
...
<script>
    $(document).on("click",".fa.fa-star",function(){
       ...
       $.post("insert_seller_rating.jsp",{id:v,rel:u},function(data){
           });
    });
...
<body>
	...
	<div class="header-bot">
        ...
        <%
                int Rating =0;
                ResultSet rs1=st.executeQuery("select * from seller_rating where ucode='"+ucode+"'");
                if(rs1.next()){
        ...
        %>
                    <div class="col-md-6" style="margin-left:300px;margin-top: 50px">  
                        ...
                        %>
                        <span class="fa fa-star" id="<%=i%>" rel="<%=scode%>" style="color:blue"></span>
                        <%
                            }
                            for(int i=Rating+1;i<=5;i++){
                        %>
                        <span class="fa fa-star" id="<%=i%>" rel="<%=scode%>" style="color:black"></span>
                        <%
                        }
                        %>
                    </div>
        ...
        <div class="col-md-6" style="margin-left:300px;margin-top: 50px">  
           <h1>Rate Your Seller</h1> 
            <span class="fa fa-star" id="1" rel="<%=scode%>"></span>
            <span class="fa fa-star" id="2" rel="<%=scode%>"></span>
            <span class="fa fa-star" id="3" rel="<%=scode%>"></span>
            <span class="fa fa-star" id="4" rel="<%=scode%>"></span>
            <span class="fa fa-star" id="5" rel="<%=scode%>"></span>
        </div>
        <%
        }
        String Title="";
        String Description="";
        ResultSet rs2=st.executeQuery("select * from seller_Review where ucode='"+ucode+"' AND scode='"+scode+"'");
        if(rs2.next()){
        ...
        %>
        <div class="col-md-6" style="margin-left:300px;margin-top: 50px">                                   
            <form  method="post" action="insert_seller_Review.jsp?ucode=<%=ucode%>&scode=<%=scode%>">
                
                    <h1>Review Your Seller</h1>
